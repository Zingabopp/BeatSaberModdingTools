...
{
    public class RawProjectModel : IProjectModel
    {
        public static async Task<RawProjectModel> CreateAsync(ICOMProject comProject, string projectPath)
        {
            string userProjectPath = projectPath + ".user";
            RawProjectModel model = new RawProjectModel(comProject, projectPath, userProjectPath);

            var created = await model.RefreshModel().ConfigureAwait(false);
            return (RawProjectModel)created;
        }
        ...
        public RawProjectModel(ICOMProject comProject, XDocument project, string projectPath, XDocument userProject, string userProjectPath)
        {
        ...
        private RawProjectModel(ICOMProject comProject, string projectPath, string? userProjectPath = null)
        {
            COMProject = comProject ?? throw new ArgumentNullException(nameof(comProject));
            // Set in CreateAsync
            Project = null!;
